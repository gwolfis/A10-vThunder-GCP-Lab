...
3. Verify the slb config on the ADC if server, service-group and virtual server are up.  
...
* **`Permission denied` / `iam.serviceAccounts.create` / `storage.buckets.create`**
  → The Terraform deployment service account is missing the required IAM roles, see `getting-started` section and additional GCP docs.
